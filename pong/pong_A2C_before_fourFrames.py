...
class ActorCritic():
    def __init__(self, actor, critic, feature, num_envs, num_frames, alpha=1e-4, gamma=0.99):
        ...
        self.alpha = alpha
        self.gamma = gamma
        self.actor_optim = AdamW(self.actor.parameters(), lr=1e-2)
        self.critic_optim = AdamW(self.critic.parameters(), lr=1e-2)
        self.feature_optim = AdamW(self.feature.parameters(), lr=1e-2)
        self.stats = {'Actor Loss': [], 'Critic Loss': [], 'Returns': []}
        self.best_return = -9999
        self.device = device

        # for debugging why agent is not learning----#
        self.actor_actions = []                      #
        self.input_states = []                       #
        # -------------------------------------------#
    ...
    def train(self, env, episodes):
        for episode in tqdm(range(1, episodes + 1)):
            ...
            while not done_b.all():
                ...
                action_probs = self.actor(state_f)
                ...
                actor_loss = - I * action_log_prob * advantage - 0.001 * entropy
        ...
        # for save and plot actor's action histogram-----#
        self.save_action_decision()                      #
        # -----------------------------------------------#

        # for save and plot input state after proprocess---#
